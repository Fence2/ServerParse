...
class Parser:
    ...
    def start(
        ...
        if len(config_components):
            config_components = westcomp.data_prettify.prettify_components_cfg(config_components)

        print("Уникальных комплектующих из конфигураторов:", len(config_components))

        if len(servers_with_configs):
            servers_with_configs = westcomp.data_prettify.prettify_servers(servers_with_configs)
