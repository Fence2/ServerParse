...
class Parser:
    ...
    def start(
        ...
        if get_new_components:
            ...
            catalog = servergate.catalog.Catalog()
        ...
        if get_new_servers_list:
            ...
            catalog = servergate.catalog.Catalog(self.webdriver_path, False) if catalog is None else catalog
        ...
        if get_new_servers_configs:
            configurator = servergate.configurator.Configurator(self.webdriver_path)
        ...
        if len(config_components):
            config_components = servergate.data_prettify.prettify_components_cfg(config_components)

        print("Уникальных комплектующих из конфигураторов:", len(config_components))

        if len(servers_with_configs):
            servers_with_configs = servergate.data_prettify.prettify_servers(servers_with_configs)
